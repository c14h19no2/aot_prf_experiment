...
class PRFBarPassSession(PylinkEyetrackerSession):
    def __init__(self, output_str, output_dir, settings_file, eyetracker_on=True):
        ...
        stim_file_path = os.path.join(
            ...
            "stimuli",
            self.settings["stimuli"].get("bg_stim_h5file"),
        )
    ...
    def create_fixation_mark_times(self):
        ...
        nr_events = int(
            6 * self.total_time / self.settings["design"].get("minimal_ifi_duration")
        )
        exponentials = np.random.exponential(
            self.settings["design"].get("exponential_ifi_mean"), nr_events
        )
        gaussians = np.random.randn(nr_events) * self.settings["design"].get(
            "gaussian_ifi_sd"
        )
        offsets = np.ones(nr_events) * self.settings["design"].get(
            "offset_ifi_duration"
        )
        minimum = self.settings["design"].get("minimal_ifi_duration")

        self.fix_event_durations = exponentials + gaussians + offsets
        self.fix_event_durations = self.fix_event_durations[
            self.fix_event_durations > minimum
        ]  # shouldn't be too restrictive
        self.fix_event_durations[self.fix_event_durations < offsets] = self.settings[
            "design"
    ...
    def bar_stimulus_lookup(self, bar_width, bar_direction, bar_refresh_time):
        ...
        return {
            "bar_display_frames": bar_display_frames,
            "bg_stim_frames": bg_stim_frames,
        }
            ...
            for j in range(len(bar_refresh_times)):
                for k, bd in enumerate(bar_directions):
                    ...
                    parameters = {
                        ...
                        "bar_blank_duration": self.settings["stimuli"].get(
                            "bar_blank_duration"
                        ),
                        "bar_blank_interval": self.settings["stimuli"].get(
                            "bar_blank_interval"
                        ),
                        "bg_stim_refresh_time": self.settings["stimuli"].get(
                            "bg_stim_refresh_time"
                        ),
                    }
                    if bd == -1:
                        self.trials.append(
    ...
    def close(self):
        ...
        for trial in self.trials:
            if type(trial) == BarPassTrial:
                ...
                trial.aperture_sequence_df.to_hdf(
                    h5_seq_file,
                    key=f"trial_{str(trial.trial_nr).zfill(3)}/apertures",
                    mode="a",
                )
        ...
        np.savetxt(
            os.path.join(self.output_dir, self.output_str + "_fix_responses.tsv"),
            true_fix_events,
            delimiter="\t",
        )
